...
# tink-go-hcvault dependencies, i.e., tink-go.

set -euo pipefail

if [[ -n "${KOKORO_ROOT:-}" ]]; then
  cd "${KOKORO_ARTIFACTS_DIR}/git/tink_go_hcvault"
...
#  ${KOKORO_ARTIFACTS_DIR}/git/tink_go_hcvault
#
# If this is not the case, we are using this script locally for a manual one-off
# test (running it from the root of a local copy of the tink-go-hcvault repo).
: "${TINK_BASE_DIR:=$(pwd)/..}"
